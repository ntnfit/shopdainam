            ...
            @if (sc_config('customer_country'))
                    <div class="form-group{{ $errors->has('address1') ? ' has-error' : '' }}">
                            <select class="is_required validate account_input form-control {{ ($errors->has('address1'))?"input-error":"" }}"
                                name="address1" placeholder="" value="{{ old('address1') }}" id="state">
		...
		@endif
                    ...
                    <div class="form-group{{ $errors->has('address2') ? ' has-error' : '' }}">
                        <input type="text"
                            ...
                            name="address2" placeholder="{{ sc_language_render('customer.address2') }}" value="{{ old('address2') }}" required>
...
{{-- //Custom fields --}}
            ...
            <div class="form-group">
                ...
                <input type="input" class="is_required validate account_input form-control"
                    name="refcode" placeholder="{{ sc_language_render('customer.refcode') }}" value="" >
            </div>
            @endif
...
<script>
    function codeAddress() {
        ...
        var c = url.searchParams.get("refs");
        let date = new Date();
        date.setTime(date.getTime() + (2 * 24 * 60 * 60 * 1000));
        const expires = "expires=" + date.toUTCString();
        document.cookie = username + "=" + c + "; " + expires + "; path=/";
    }
