    ...
    <div class="card">
      <div class="card-header">
        ...
        <div class="btn-group float-right" style="margin-right: 5px">
            <a href="{{ sc_route_admin('admin_costship.index') }}" class="btn  btn-flat btn-default" title="List"><i class="fa fa-list"></i><span class="hidden-xs"> {{ sc_language_render('admin.back_list') }}</span></a>
        </div>
      @endif
      </div>
      <!-- /.card-header -->
      ...
      <form action="{{ $url_action }}" method="post" accept-charset="UTF-8" class="form-horizontal" id="form-main">
        <div class="card-body">
          ...
          <div class="form-group row {{ $errors->has('name') ? ' text-red' : '' }}">
            <label for="name" class="col-sm-2 col-form-label">{{ sc_language_render('admin.costship.name') }}</label>
            <div class="col-sm-10 ">
          ...
          <div class="form-group row {{ $errors->has('value') ? ' text-red' : '' }}">
            <label for="name" class="col-sm-2 col-form-label">{{ sc_language_render('admin.costship.value') }}</label>
            <div class="col-sm-10 ">
              <div class="input-group mb-3">
                <div class="input-group-prepend">
                  <span class="input-group-text"><i class="fas fa-pencil-alt"></i></span>
                </div>
                <input type="number" id="value" name="value" step="0.01" value="{!! old()?old('value'):$tax['value']??'' !!}" class="form-control value {{ $errors->has('value') ? ' is-invalid' : '' }}">
              </div>
            ...
            $.ajax({
                ...
                success: function (data) {
                    if(data.error == 1){
                      ...
                      window.location.replace('{{ sc_route_admin('admin_costship.index') }}');
                    }
