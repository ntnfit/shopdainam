...
class ShopAccountController extends RootFrontController
{
 ...
 public function getReferral (...$params)
    {
        if (config('app.seoLang')) {
            $lang = $params[0] ?? '';
            sc_lang_switch($lang);
    ...
    private function _Referral()
    {
        $customer = auth()->user();
        $id = $customer->email;
        $referral = DB::table('referral_relationships')->where('referral_link_id', $id)->get();
        sc_check_view($this->templatePath . '.account.referral');
        return view($this->templatePath . '.account.referral')
            ->with(
                [
                'title'       => sc_language_render('referral'),
                'customer'    => $customer,
                'referral'    =>$referral,
                'refs'        => base64_encode($customer->email),
                'layout_page' => 'shop_profile',
                'breadcrumbs' => [
                    ['url'    => sc_route('customer.index'), 'title' => sc_language_render('front.my_account')],
                    ['url'    => '', 'title' => sc_language_render('referral')],
                ],
                ]
            );
    }
